...
function bunnypresslite_customizer_add_style(){
	...
	echo '@media screen and (max-width: 560px){';
	...
	echo '@media screen and (max-width: 560px){.sitename a{padding:' . absint( $bunnypresslite_sptfs / 100 ) . 'em 0;}.footermenu ul li{background:rgba(' . esc_attr(bunnypresslite_color_conversion( $ffc) ) . ',.1);border-bottom:1px solid rgba(' . esc_attr(bunnypresslite_color_conversion( $ffc )) . ',.3);}.footermenu ul{border-top:1px solid rgba(' . esc_attr(bunnypresslite_color_conversion( $ffc) ) . ',.3);}}';
	...
	if( is_user_logged_in() ) echo '#wpadminbar{position:fixed!important}';
	echo "</style>\n";
